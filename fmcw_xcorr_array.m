function [iw,ic,cc,lags,pe,pse] = fmcw_xcorr_array(f,g,fi,maxlag,fe,ge,p)
    
% [iw,ic,cc,lags,pe,pse] = fmcw_xcorr_array(f,g,fi,maxlag,fe,ge,p)
%
...
for ii = 1:length(lags) % loop through offsets
    ...
    % Rewriting mean function...
    fga = abs(fg(fg==fg)); % Remove NaNs from abs(fg)
    fix = fi(fg==fg); % Remove NaNs from fi
    iw(ii) = (((sum(fga.*fix))/length(fga)) / (sum(fga)/length(fga))) - n;
